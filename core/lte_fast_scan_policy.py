...
def _create_lte_instrument_group(self: LeftPanel):
    ...
    # Legacy fields remain internally for compatibility but are no longer operator settings.
    self.sensitivity_upper_spin.setVisible(False)
    self.stop_level_spin.setVisible(False)
    if hasattr(form, "labelForField"):
        for widget in (self.sensitivity_upper_spin, self.stop_level_spin):
...
def _create_lte_channel_group(self: LeftPanel):
    ...
    selected = {item for item in str(_setting(self, "test_items", "三信道测试", str)).split("|") if item}
    for name, checkbox in self.lte_test_item_checkboxes.items(): checkbox.setChecked(name in selected)
    return group

def _create_lte_band_group(self: LeftPanel):
    group = _original_create_lte_band_group(self)
    selected = {item for item in str(_setting(self, "bands", "", str)).split("|") if item}
    for band, checkbox in self.band_checkboxes.items(): checkbox.setChecked(f"B{band}" in selected)
    return group

def _save_lte_settings(self: LeftPanel, config: Any) -> None:
    values = {
        "cable_loss": config.cable_loss, "start_level": config.start_level,
        "packet_count": config.packet_count, "fast_packet_count": getattr(config, "fast_packet_count", FAST_PACKET_DEFAULT),
        "max_step": config.max_step, "min_step": config.min_step, "bler_threshold": config.bler_threshold,
        "settle_time": config.settle_time, "retry_count": config.retry_count, "com_port": config.com_port,
        ...
        "bands": "|".join(config.selected_bands), "test_items": "|".join(config.lte_test_items),
    }
    for key, value in values.items(): self.settings.setValue(f"{SETTINGS_PREFIX}{key}", value)
    self.settings.remove(f"{SETTINGS_PREFIX}sensitivity_upper")
    self.settings.sync()

def _collect_lte_config(self: LeftPanel):
    config = _original_collect_lte_config(self)
    # Keep the legacy field synchronized only for old code paths; it is no longer a limit.
    config.sensitivity_upper = config.start_level
...
def _wait_connected(worker: TestWorker, timeout: float) -> bool:
    ...
    worker._raise_if_stopped(); worker._emit_instrument_warning(); return bool(result)
...
def _prepare_cell(self: TestWorker, item: Any) -> None:
    ...
    self._call_lte_prepare_cell(item); self._raise_if_stopped(); self._emit_instrument_warning()
    ...
    for attempt in range(1, ATTACH_ATTEMPTS + 1):
        ...
        self._call_lte_cell_on(item); self._raise_if_stopped(); self._emit_instrument_warning()
        self.set_state(TestState.WAITING_ATTACH, "状态切换：WAITING_ATTACH - 等待 UE Attach")
        started = time.monotonic(); connected = self._call_wait_for_attach(); elapsed = time.monotonic() - started
        if connected:
            self._raise_if_stopped(); self._emit_instrument_warning(); self.set_state(TestState.ATTACHED, "状态切换：ATTACHED - UE 已连接")
            self.log_signal.emit("INFO", f"UE 已连接，耗时 {elapsed:.2f} s"); self._run_before_measure(); return
...
def _ensure_connected_for_probe(worker: TestWorker, item: Any, requested_level: float) -> tuple[float, bool]:
    level = float(requested_level); instrument_level = worker._instrument_level_for_dut(item, level)
    worker.instrument.set_rx_level(instrument_level); worker._raise_if_stopped(); worker._emit_instrument_warning()
    if _wait_connected(worker, 0.0): return level, False
    boost = float(getattr(worker.config, "reconnect_boost_db", RECONNECT_BOOST_DB)); attempts = int(getattr(worker.config, "reconnect_attempts", RECONNECT_ATTEMPTS))
    worker.log_signal.emit("WARNING", f"{item.band}/{item.channel} UE 已掉线，开始 +{boost:g} dB 恢复")
    recovery_level = level
    for attempt in range(1, attempts + 1):
        recovery_level = round(recovery_level + boost, 10); worker.instrument.set_rx_level(worker._instrument_level_for_dut(item, recovery_level)); worker._raise_if_stopped(); worker._emit_instrument_warning(); started = time.monotonic()
        if _wait_connected(worker, 10.0): worker.log_signal.emit("INFO", f"UE 已恢复连接，耗时 {time.monotonic() - started:.2f} s"); return recovery_level, True
...
def _query_full_cell_bw_power(worker: TestWorker) -> float | None:
    ...
    try:
        response = str(query(FULL_CELL_BW_POWER_QUERY)).strip(); match = re.search(r"[-+]?\d+(?:\.\d+)?(?:[Ee][-+]?\d+)?", response)
        if not match: raise ValueError(f"无法解析仪表返回值：{response!r}")
        return float(match.group(0))
    except Exception as exc:
        worker.log_signal.emit("WARNING", f"Full Cell BW Power 查询失败：{exc}"); return None

def _measure_with_packets(worker: TestWorker, item: Any, level: float, phase: str, current: int, total: int, packet_count: int) -> bool:
    original_packets = worker.config.packet_count; original_retries = worker.config.retry_count
    worker.config.packet_count = int(packet_count); worker.config.retry_count = 0
    try:
        passed = _original_measure_level(worker, item, level, phase, current, total); full_cell_bw_power = _query_full_cell_bw_power(worker)
        if full_cell_bw_power is not None: worker.log_signal.emit("INFO", f"{phase} {item.band}/{item.channel} Full Cell BW Power={full_cell_bw_power:g} dBm")
        return passed
    finally:
        worker.config.packet_count = original_packets; worker.config.retry_count = original_retries

def _scan_item(self: TestWorker, item: Any, current: int, total: int) -> None:
    level = float(self.config.start_level); fast_packets = int(getattr(self.config, "fast_packet_count", FAST_PACKET_DEFAULT)); formal_packets = int(self.config.packet_count); max_step = float(self.config.max_step); min_step = float(self.config.min_step)
    ...
    while True:
        ...
        if _measure_with_packets(self, item, level, "FAST", current, total, fast_packets): level = round(level - max_step, 10); continue
        confirm_level, recovered = _ensure_connected_for_probe(self, item, level)
        if recovered: level = confirm_level; continue
        if _measure_with_packets(self, item, level, "CONFIRM", current, total, formal_packets): level = round(level - max_step, 10); continue
        confirmed_fail_level = level; fine_level = round(confirmed_fail_level + min_step, 10)
        while True:
            fine_level, recovered = _ensure_connected_for_probe(self, item, fine_level)
            if recovered: level = fine_level; break
            if _measure_with_packets(self, item, fine_level, "FINE", current, total, formal_packets):
                self.log_signal.emit("INFO", f"Sensitivity 边界：PASS={fine_level:g} dBm，FAIL={confirmed_fail_level:g} dBm"); return
            confirmed_fail_level = fine_level; fine_level = round(fine_level + min_step, 10)
